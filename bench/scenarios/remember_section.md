...
3. **Remember trace** — Child file shows tool-call markdown, failure-only
   `tool-result` fences when patches miss, and/or bounded `<!-- remember:`
   commentary rather than raw narrative leakage
...
6. **Failure visibility** — Failed remember patches surface in logs and/or
   child `tool-result` fences; avoid repeated identical patch calls after success
