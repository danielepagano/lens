#!/usr/bin/env bash
# Canonical Setup for play_module_request — keep in sync with bench/scenarios/play_module_request.md
# Usage (repo root): export PROJECT && bash bench/scenarios/play_module_request_setup.sh
set -euo pipefail
if [[ -z "${PROJECT:-}" ]]; then
  echo "play_module_request_setup.sh: export PROJECT to the bench project directory first" >&2
  exit 1
fi
cd "$PROJECT"
