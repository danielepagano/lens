...
1. **Create the project** with `bench/tools/setup_bench.py` (see `bench/agent.md`)
...
6. **Report (required):** run `report.py init --scenario … --profile … --project-dir "$PROJECT"` (see `bench/agent.md`).
7. **Complete the report:** fill `steps` and `evaluation` — preferably **`python bench/tools/report.py merge bench/reports/your_report.json < patch.json`** (updates JSON and HTML together), or hand-edit JSON then **`report.py render`** or **`report.py sync`** so HTML is never stale.
