...
Roll the damage dice specified. For weapons, add the relevant ability modifier. Most spell damage rolls do not add an ability modifier unless the spell text says so.
...
KNOCKING OUT: When a melee attack would reduce a target to 0 HP, the attacker may choose to knock out instead → reduce to 1 HP, Unconscious, begins a Short Rest.
...
STUNNED: Has Incapacitated. Auto-fail Str and Dex saves. Attacks against you: Advantage.
