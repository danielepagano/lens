...
Establish the setting pressure and narrative weather for a new game. This produces `lore.world` — a concise, character-agnostic object pinned into EVERY play interaction. It is the AI's primary voice and atmosphere guide.
...
Start by checking whether the setting already has a spine. Use `kb_get` to check if a setting lore object already exists (e.g. `lore.grim-hollow`, or any `lore.*` the user mentions). If one exists, use it as the baseline — don't reinvent what the dataset already provides. Ask the user what they want to keep, change, or emphasize.

If no baseline exists, or the user wants something custom, establish the frame:
...
Write `lore.world` as guidance the GM can inhabit. The object must be under 500 words. It will be in the prompt of every play interaction, so every word costs tokens. Write it as a directive, not a description:
...
What this is not:
- Not a wiki introduction.
- Not a pile of lore trivia.
- Not a hidden plot outline disguised as worldbuilding.
