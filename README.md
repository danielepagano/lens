...
git init               # if new
lens init              # lens.toml, knowledge/, narrative/, agent skill pointer
```

`lens init` also writes `.claude/skills/lens/SKILL.md`, which tells a coding agent
working in the repo to run **`lens skill`** — guidance generated by the installed
Lens against this project, rather than a description committed once and left to go
stale. Skip it with `--no-skill`.
