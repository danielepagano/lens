...
CMD_RELEASE = (
    "Manage release-managed deployments (when `[release] enabled`).\n\n"
    "`lens release init` installs the CI trigger files (and bootstraps\n"
    "the Fly app when needed). `lens release add` / `remove` keep the Fly\n"
    "secrets and `[[dependent_project]]` topology in sync, `lens release push`\n"
    "redeploys the shared image, and `check` / `apply` drive the parent-hash\n"
    "gate. Use `lens deploy` for desktop-only deployments that never enable\n"
    "the release system. See deploy/README.md and docs/configuration.md for\n"
    "the complete split between release and desktop flows."
)
...
RELEASE_INIT = (
    ...
    "When no ``fly.toml`` exists yet, this command can also bootstrap the\n"
    "Fly app (same flags as ``lens deploy init``) before installing the\n"
    "CI files. Use ``--leader`` to designate this project as the release\n"
    "leader for a multi-project Fly app. Run ``lens release check``\n"
