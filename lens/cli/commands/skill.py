...
@app.callback()
def skill(
    topic: str | None = typer.Argument(None, help=ARG_SKILL_TOPIC),
    ...
    if topic is not None and (install or check or sources):
        typer.echo("lens skill: a topic cannot be combined with other options", err=True)
        raise typer.Exit(1)
    ...
    if topic is not None:
        if project_root is None:
            typer.echo("lens skill: topics come from a project's datasets; none here", err=True)
            raise typer.Exit(1)
        try:
            typer.echo(render_topic(project_root, topic), nl=False)
    ...
    if sources:
        for layer in collect_layers(project_root):
    ...
    typer.echo(render_guidance(project_root), nl=False)
