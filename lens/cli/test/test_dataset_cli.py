...
class TestDatasetCLI(unittest.TestCase):
    ...
    def test_pin_blocked(self) -> None:
        result = subprocess.run(
            _LENS_CMD + ["pin", "kb", "add", "person.hero"],
            cwd=self.dataset_dir,
            capture_output=True,
            text=True,
        )
...
class TestKbEditCLI(unittest.TestCase):
    ...
    def test_edit_help(self) -> None:
        ...
        self.assertRegex(out, r"-{1,2}context\b")
        self.assertRegex(out, r"-{1,2}pin\b")
        self.assertRegex(out, r"-{1,2}unpin\b")
        self.assertRegex(out, r"-{1,2}include-template\b")
        self.assertRegex(out, r"-{1,2}llm\b")
