...
Dataset-gated commands (from dataset extensions, e.g. `lens dnd` when `lens-dnd` is listed) and operators (`play`, `advance` when `rpg` is listed) appear only when the dataset is in `[project] datasets` and resolves. `lens --help` groups everything into panels — **Project**, **Operators**, **Knowledge**, **Media**, **Dataset commands**, **Serving & deploy** — in that reading order; a command module opts into one with a module-level `help_panel` string, and `LensGroup` in `lens/cli/main.py` fixes the order the panels print in (Rich would otherwise order them by registration accident). See [RPG](../rpg/README.md) and [datasets/README.md](../../datasets/README.md#dataset-extensions-optional-python).

## Project commands

### `lens init`

Initialize a Lens project in the current git repo. Creates `lens.toml` (with a comment pointing to `lens use`), `knowledge/`, `narrative/`, and `.claude/skills/lens/SKILL.md` (see [`lens skill`](#agent-onboarding-lens-skill)). Does not set the active narrative or add LLM configuration. Requires an existing git repository.

- `--no-skill` — Skip the agent skill pointer.
...
Also reports whether the committed agent skill pointer matches what this Lens would write (`warn` only — a lagging pointer is a stale document, not a broken project).

### `lens stats`

Count knowledge objects by type (and how many live in this repository), list the model-requestable modules the active datasets register, and list narrative trees with node counts. Shows the active narrative cursor, the pins in effect there, and whether an open transaction exists (and its owner). This is the inventory `lens skill` deliberately leaves out.
...
## Agent onboarding (`lens skill`)

Print what an agent needs to know about *this* project, generated now.

An agent that checks out a Lens project sees `lens.toml`, `knowledge/`, `narrative/`
and a CLI it has to discover — and the parts most likely to bite it (the store is a
merge, tags live outside the files, a dataset file is shared) are exactly the parts
no `--help` mentions. Anything descriptive committed into a content repo goes stale
the moment Lens moves, silently. So the repo gets a **pointer**, and the guidance is
generated by the installed Lens on demand.

```bash
lens skill              # print the current guidance
lens skill rpg          # print one active dataset's full conventions
lens skill --sources    # list the layers and where each came from
lens skill --install    # write .claude/skills/lens/SKILL.md (the pointer)
lens skill --check      # exit 1 if the installed pointer has drifted
```

Unlike every other command, `lens skill` runs outside the preflight: no project, a
dataset checkout, or no active narrative all still print — those are exactly the
moments it is asked.

The output composes four layers, later ones appended rather than shadowing:

| Layer | Owns | Where |
|---|---|---|
| Bundled | Invariants true in every Lens project | `lens/skill/guidance.md` |
| Generated | Where the cursor is (and any session open there); which datasets merge in, and from where | — |
| Dataset | Its gist, up to a `<!-- more -->` line | `<dataset>/skill/skill.md` |
| Project | House rules, last so they win | `skill/skill.md` |

It is read once, before work starts, and often through a tool that truncates long
output, so it leaves out anything another command answers. The command surface is
`lens --help` (dataset gating applied). Types, counts, narratives and
model-requestable modules are `lens stats`. Tags are `lens kb list-tags`, design
modules `lens kb list --type design`, and local forks
`lens kb list --shadowed --source project`. How to drive a single command is that
command's `--help`.

A dataset's full conventions are a topic: `lens skill <dataset>` prints the whole
file, gist included. A dataset file without the marker is emitted whole in the main
output. In a dataset checkout the dataset's own file is the layer being edited, so
it is emitted whole there too.

`--install` writes only the pointer: frontmatter so a host discovers it unprompted,
"run `lens skill`", and the two or three invariants worth stating even when `lens`
cannot be run at all. It carries no project facts, so `--check` compares it byte for
byte and drift means "Lens changed what it says", never "somebody added an object".
`lens init` installs it by default; `lens check` reports drift as a warning.
