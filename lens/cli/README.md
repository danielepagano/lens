...
| Generated | This project's datasets, types, tags, modules, forks, **and command surface** | — |
| Dataset | Its own conventions | `<dataset>/skill/skill.md` |
| Project | House rules, last so they win | `skill/skill.md` |

The command listing is introspected from the Typer app that is about to handle the
invocation (`lens/cli/command_inventory.py`), not written down — so it is exactly
what `lens --help` would print, with dataset gating already applied: an operator or
extension command absent from the listing does not exist in this project. Groups
list their sub-command *names* only; options stay behind `lens <cmd> --help`, which
is always current and which no generated listing can beat.
