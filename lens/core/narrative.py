...
def _read_tail_cursor_annotation(path: Path) -> ParsedAnnotation | None:
    ...
    Seeks to the last 1024 bytes so large node files are never fully read just
    to discover the cursor position.  Long multi-line annotations can exceed
    that window, so fall back to the full file when the visible tail looks like
    the end of an annotation block but the opener was outside the chunk.
    """
    try:
    ...
    if ann is not None or size <= 1024:
        return ann
    if text.rstrip().endswith("]: #"):
        try:
            return parse_tail_cursor_annotation(path.read_text(encoding="utf-8"))
        except OSError:
            return None
    return None
