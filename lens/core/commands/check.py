...
def run_project_check(project_root: Path, *, skip_network: bool = False) -> ProjectCheckResult:
    ...
    if isinstance(raw_mount, str) and raw_mount.strip():
        mp = raw_mount.strip()
        cloud_mount_error: str | None = None
        try:
            require_cloud_compatible_mount(project_root)
        except LensException as exc:
            cloud_mount_error = str(exc)
        if cloud_mount_error:
            result.add("error", "mount", cloud_mount_error)
        elif mp.startswith("s3://"):
    ...
    for name in ds_names:
        ...
        if resolved is None:
            ...
            result.add(
                "error",
                f"dataset {name}",
                f"not found (checked bundled {bundled}, sibling, lens.local.toml)",
            )
            continue
        result.add("ok", f"dataset {name}", str(resolved))
        ext_err = try_load_dataset_extension(project_root, name, resolved)
        if ext_err is not None:
            result.add("warn", f"dataset {name} extension", ext_err)
