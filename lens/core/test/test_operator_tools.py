...
class TestPlayOperatorRequirements(unittest.TestCase):
    def test_no_pins_raises(self) -> None:
        ...
        self.assertIn("rules.rpg", str(ctx.exception))
    ...
    def test_missing_pc_pin_raises(self) -> None:
        ...
        with self.assertRaises(OperatorError) as ctx:
            PlayOperator.check_requirements(_play_result(["rules.system", "rules.rpg"]))
...
class TestPlayOperatorEnrichParams(unittest.TestCase):
    def test_enrich_params_sets_pc_key_from_first_pinned_pc(self) -> None:
        result = _play_result(
            ["rules.system", "rules.rpg", "pc.alice", "pc.bob"]
        )
...
class TestWritePlayToolChain(unittest.TestCase):
    ...
    def test_write_has_no_operator_tools(self) -> None:
        """Write disables operator tools — LLM generates inline text only."""
        self.assertFalse(WriteOperator.use_operator_tools)

    def test_play_has_no_operator_tools(self) -> None:
        """Play disables operator tools — LLM generates inline text only."""
        self.assertFalse(PlayOperator.use_operator_tools)
