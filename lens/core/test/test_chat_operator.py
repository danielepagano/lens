...
class TestChatSession(unittest.TestCase):
    ...
    def test_fresh_session_slug_includes_character_keys(self) -> None:
        """Session ID includes character keys and optional stage directions."""
        ...
        # Slug should be chat-bob-amy-at-the
        keys = [k for k in self.narrative.child_keys() if k.startswith("chat")]
        self.assertEqual(len(keys), 1)
        self.assertIn("bob", keys[0])
        self.assertIn("amy", keys[0])

    def test_fresh_session_creates_subnode(self) -> None:
        """Starting a session creates a chat sub-node."""
        with patch("lens.core.operator.generate_text", _mock_generate):
            asyncio.run(
                ChatOperator.run_session(
                    session=self.session,
    ...
    def test_fresh_session_pins_as_character(self) -> None:
        """The --as character is pinned in the sub-node front matter."""
        with patch("lens.core.operator.generate_text", _mock_generate):
            asyncio.run(
                ChatOperator.run_session(
                    session=self.session,
    ...
    def test_fresh_session_ai_response_written(self) -> None:
    ...
    def test_fresh_session_stores_as_kb_id_in_annotation(self) -> None:
        """The as_kb_id is stored in the annotation params for later derivation."""
        with patch("lens.core.operator.generate_text", _mock_generate):
            asyncio.run(
                ChatOperator.run_session(
                    session=self.session,
    ...
    def test_fresh_session_with_stage_directions(self) -> None:
    ...
    def test_fresh_session_requires_as(self) -> None:
    ...
    def test_with_line_uses_user_label_when_no_with(self) -> None:
        """Without --with, user lines are labelled [User]."""
        with patch("lens.core.operator.generate_text", _mock_generate):
            asyncio.run(
                ChatOperator.run_session(
                    session=self.session,
                    ...
                    pins=["npc.bob"],
                    unpins=[],
                    extra_params={"as_kb_id": "npc.bob"},  # no with_kb_id
                )
            )
        Storage(self.root).stage_all()
        KnowledgeStore.clear_registry()
