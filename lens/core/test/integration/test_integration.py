...
# ---------------------------------------------------------------------------
# Section-start-with-write-chain
# ---------------------------------------------------------------------------


class TestSectionStartWithWriteChain(unittest.TestCase):
    """section start --write chains a write into the new child node."""
    ...
    def setUpClass(cls) -> None:
        ...
        cls._orig_cwd = Path.cwd()
        cls._session = setup_test_project(cls._project_dir, cls._server.base_url)
        os.chdir(cls._project_dir)
    ...
    def test_section_start_with_write_chains_write(self) -> None:
        ...
        _run_async(SectionOperator.run_start(
            session=session, narrative=narrative,
            id="ch1", write_prompt="opening scene",
        ))

        parent_text = (self._project_dir / "narrative" / "story" / "_node.md").read_text()
        self.assertIn("[section:ch1]: #", parent_text)
