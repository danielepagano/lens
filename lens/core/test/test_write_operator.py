...
class TestWriteOperatorBuildMessages(unittest.TestCase):
    def test_build_messages_assembles_sections(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(messages[0]["content"].startswith(prompts.get("write.system")))
            self.assertIn(
                "FORMATTING: you must emit valid Markdown",
                messages[0]["content"],
            )
            self.assertIn("ai:secret:", messages[0]["content"])
    ...
    def test_build_messages_system_prompt_present(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root, narrative = _make_project(_init_repo(Path(tmp)))
            op = WriteOperator(Storage(root), narrative)
            cr = CrawlResult.from_text_fields(knowledge=[], previous_summaries=[], current_content=None)
...
class TestWriteOperatorRunInline(unittest.TestCase):
    ...
    def test_run_inline_retry_llm_error_restores_prior_content(self) -> None:
        """Failed --retry must not leave the file empty after write_discard."""
        with tempfile.TemporaryDirectory() as tmp:
            root, narrative = _make_project(_init_repo(Path(tmp)))
            node_file = narrative.find_cursor().md_path()
            ...
            with self.assertRaises(OperatorError):
                _run_inline(root, narrative, retry=True, generate_mock=_fail)
