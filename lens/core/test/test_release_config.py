...
class TestParseReleaseConfig(unittest.TestCase):
    ...
    def test_all_fields_parsed(self) -> None:
        raw = {
            "release": {
                ...
                "data_major_version": 3,
                "migration_pending": True,
                "migration_target_version": "v2.0.0",
        ...
        self.assertEqual(cfg.data_major_version, 3)
        self.assertTrue(cfg.migration_pending)
        self.assertEqual(cfg.migration_target_version, "v2.0.0")
        self.assertEqual(cfg.migration_commit, "abc123")
    ...
    def test_partial_fields_use_defaults(self) -> None:
        ...
        self.assertEqual(cfg.auto_update, "off")
        self.assertEqual(cfg.data_major_version, 1)
        self.assertFalse(cfg.migration_pending)
...
class TestValidateReleaseConfig(unittest.TestCase):
    ...
    def test_data_major_version_negative_error(self) -> None:
        cfg = ReleaseConfig(
            enabled=True, lens_repo_url="https://github.com/user/lens.git", data_major_version=-1
        )
        lines = validate_release_config(cfg, [], [])
        errors = [ln for ln in lines if ln[0] == "error"]
        self.assertTrue(any("data_major_version" in ln[1] for ln in errors))
