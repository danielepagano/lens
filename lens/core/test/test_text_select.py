...
class TestResolveAmbiguity(unittest.TestCase):
    ...
    def test_identical_duplicate_lines_require_context(self) -> None:
        text = "marker\n- first\nmarker\n- second\n"
        with self.assertRaises(SelectionError) as ctx:
            resolve_selection(text, _range(_sel("marker")))
        self.assertIn("ambiguous", str(ctx.exception))

    def test_identical_duplicate_lines_disambiguated_by_neighbor(self) -> None:
        text = "marker\n- first\nmarker\n- second\n"
        resolved = resolve_selection(
            text, _range(_sel("marker", after=("- second",)))
        )
        self.assertEqual((resolved.slice_start, resolved.slice_end), (2, 3))

    def test_line_trailing_space_still_matches(self) -> None:
        text = "alpha   \nbeta\n"
        resolved = resolve_selection(text, _range(_sel("alpha")))
        self.assertEqual((resolved.slice_start, resolved.slice_end), (0, 1))

    def test_target_not_used_as_own_before_context(self) -> None:
        text = "marker\n- item\n"
        resolved = resolve_selection(
            text,
            _range(_sel("marker", before=("marker",))),
        )
        self.assertEqual((resolved.slice_start, resolved.slice_end), (0, 1))
