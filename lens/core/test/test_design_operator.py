...
def _run_design(
    ...
    *,
    ...
    with patch(
        "lens.core.operators.design.run_llm",
...
class TestDesignIdGeneration(unittest.TestCase):
    ...
    def test_collision_adds_numeric_suffix(self) -> None:
        ...
        (folder / "design.md").write_text("existing\n")
        parent.md_path().write_text(
            parent.md_path().read_text(encoding="utf-8").rstrip()
            + "\n[design:design]: #\n",
            encoding="utf-8",
        )
    ...
    def test_multiple_collisions(self) -> None:
        parent = self._parent()
        folder = parent.md_path().parent
        (folder / "design.md").write_text("one\n")
        (folder / "design-1.md").write_text("two\n")
        parent.md_path().write_text(
            parent.md_path().read_text(encoding="utf-8").rstrip()
            + "\n[design:design]: #\n[design:design-1]: #\n",
            encoding="utf-8",
...
class TestDesignModulePin(unittest.TestCase):
    def test_module_recorded_in_parent_annotation(self) -> None:
        """Module is persisted in the parent open annotation, not front-matter pins.

        ``ModuleTransform`` reads it back at crawl time and adds the KB
        component, so there is no need to write it to the sub-node front matter.
        """
        from lens.core.annotations import parse_annotations
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            modules = [
                a.params.get("module")
                for a in parse_annotations(parent_text)
                if a.operator == "design" and a.id == session_id and not a.closing and not a.self_closing
            ]
            self.assertIn("encounter", modules)
...
class TestDesignCompanionTemplatePin(unittest.TestCase):
    """Module + ``<key>._template`` companion are added to the LLM context by
    ``ModuleTransform`` at crawl time. Neither one ends up in front-matter
    ``kb_pin`` — they live as transform-emitted graph components."""

    def _captured_user_content(
        self, root: Path, narrative: NarrativeNode, module_id: str
    ) -> str:
        captured: list[list[dict[str, str]]] = []
    ...
    def test_pins_type_template_when_kb_has_it(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            content = self._captured_user_content(root, narrative, "location")

            self.assertIn("design.location", content)
            self.assertIn("location._template", content)
    ...
    def test_no_template_pin_when_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            content = self._captured_user_content(root, narrative, "world")
    ...
    def test_module_switch_replaces_template_pin(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            content = self._captured_user_content(root, narrative, "npc")

            self.assertIn("design.npc", content)
...
class TestDesignContinueRun(unittest.TestCase):
    ...
    def test_second_call_crawls_design_child(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def capture_crawl(spec: Any, **kwargs: Any) -> Any:
                from lens.core.context import CrawlSpec, crawl as real_crawl
                node = spec.node if isinstance(spec, CrawlSpec) else spec
                crawl_called_with_node.append(node)
                return real_crawl(spec, **kwargs)

            with patch(
                "lens.core.operators.design.run_llm",
                new=mock_run_llm(_fake_generate_text),
            ):
                with patch("lens.core.command_tools.get_command_registry", return_value={}):
...
class TestDesignModuleSwitch(unittest.TestCase):
    def test_switch_module_replaces_pin(self) -> None:
        """Switching ``--module`` rewrites the parent annotation's ``module``
        param so subsequent crawls resolve the new module via
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            modules = [
                a.params.get("module")
                for a in parse_annotations(parent.md_path().read_text(encoding="utf-8"))
                if a.operator == "design"
                and a.id == session_id
                and not a.closing
