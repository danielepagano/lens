...
class TestChildDiscovery(unittest.TestCase):
    ...
    def test_child_keys_omits_unannotated_files(self) -> None:
        """Only children referenced in parent annotations are listed."""
        with tempfile.TemporaryDirectory() as tmp:
            p = Path(tmp)
            narrative = _make_narrative(p)
            for key in ("b", "c", "a"):
            ...
            node = NarrativeNode(narrative_root=narrative, key_path=())
            self.assertEqual(node.child_keys(), ["b"])
...
class TestFindCursor(unittest.TestCase):
    ...
    def test_cursor_with_long_multi_line_session_annotation(self) -> None:
        """find_cursor descends even when params push the opener outside the tail read."""
        with tempfile.TemporaryDirectory() as tmp:
            p = Path(tmp)
            narrative = _make_narrative(p)
            long_prompt = "You awaken. " + ("Long scene. " * 80)
            (narrative / "_node.md").write_text(
                "# root\n\n"
                "[chat:chat-awaken\n"
            ...
            (narrative / "chat-awaken").mkdir()
            (narrative / "chat-awaken" / "_node.md").write_text(
                "[chat]: #\n\n"
                "> **Scene:** You awaken.\n\n"
                "> [Eve] Hello.\n\n"
                "[/chat]: #\n",
            ...
            node = NarrativeNode(narrative_root=narrative, key_path=())
            cursor = node.find_cursor()
            self.assertEqual(cursor.key_path, ("chat-awaken",))
