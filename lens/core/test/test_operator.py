...
class TestMode2Subnode(unittest.TestCase):
    ...
    def test_close_subnode(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # close_subnode now appends the summary as-is; callers that
            # produce an LLM summary must pre-format via format_summary_block.
            pre_formatted = (
                "<!-- section:ch1 -->\n"
                "\n"
                "### Chapter One\n"
            ...
            op.close_subnode(narrative, "ch1", pre_formatted)
            parent_text = narrative.md_path().read_text()  # type: ignore[union-attr]
            self.assertIn(
                "<!-- section:ch1 -->\n\n### Chapter One\n\n"
                "> Summary of ch1.\n> Second line.\n\n[/testop:ch1]: #\n",
...
class TestContextAwareOperatorHelpers(unittest.TestCase):
    ...
    def test_write_start_verbatim_prefix_not_transformed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root, narrative = _make_project(_init_repo(Path(tmp)))
            owner = _ConcreteCAO.owner_id(None, "narrative/test/_node.md", line=3)
            op = _ConcreteCAO(Storage(root, owner=owner), narrative)
            op.write_start(
                ...
                "[cao]: #",
                "Assistant",
                verbatim_prefix="> User scene\n\n",
            )
            text = narrative.md_path().read_text()
            self.assertIn("> User scene", text)
            self.assertIn("Assistant", text)
            self.assertNotIn("User scene [fixed]", text)

    def test_write_append_verbatim_prefix_not_transformed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root, narrative = _make_project(_init_repo(Path(tmp)))
            owner = _ConcreteCAO.owner_id(None, "narrative/test/_node.md", line=3)
            op = _ConcreteCAO(Storage(root, owner=owner), narrative)
            ...
            op.write_append(
                ...
                ann,
                "Second",
                verbatim_prefix="> User\n\n",
            )
            text = narrative.md_path().read_text()
            self.assertIn("> User", text)
            self.assertIn("Second", text)
            self.assertNotIn("User [fixed]", text)
    ...
    def test_run_inline_injects_slice_refs_into_prompt_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                ...
                new=mock_run_llm(_capture_text),
            ):
                asyncio.run(
                    _NoIdOp.run_inline(
                        session=session,
