...
Session lifecycle
-----------------
*  First call → creates a sub-node (``play[-module][-prompt-slug]``) and runs the
   first inline generation inside it.  Core rules (``rules.system``, ``rules.rpg``)
   enter context at crawl time via the ``rpg_play_context`` modality, not sub-node
   front-matter pins.
*  Subsequent calls inside the same session → append new inline blocks.
   Front matter is updated for user ``kb_pin`` / ``kb_unpin``; ``--module``
   rewrites the parent open annotation (see Module handling).
...
REQUIRED_PINS: frozenset[str] = frozenset(PLAY_AUTO_PINS)
...
class PlayOperator(SessionOperator):
    ...
    async def _append_player_line(
        ...
        *,
        ...
        crawl_result, ctx, resolved = cls.crawl_with_modalities(
            spec, {}, session=session, narrative=narrative
        )
    ...
    async def _run_fresh(
        ...
        **kwargs: Any,
        ...
        on_status: Callable[[str], None] | None = cast(
            "Callable[[str], None] | None", kwargs.get("on_status")
        )
        on_info_event: Callable[[dict[str, Any]], Awaitable[None]] | None = cast(
            "Callable[[dict[str, Any]], Awaitable[None]] | None", kwargs.get("on_info_event")
        )
        workflow: Any | None = kwargs.get("workflow")

        appended_player_line = False
        if prompt:
            try:
                resolved = cls.transform_storable_prompt(
                    prompt, session=session, cursor=node
                )
