...
class TestPlayAppend(unittest.TestCase):
    ...
    def test_mention_kb_is_not_persisted_to_player_turn(self) -> None:
        ...
        self.assertNotIn("KB['spell.foo']", text)
        self.assertNotIn("visible line", text)
    ...
    def test_node_slice_refs_are_not_persisted_to_player_turns(self) -> None:
        ...
        text = self._play_child_md()
        self.assertNotIn("REF['test/lore@1:2']", text)
        self.assertNotIn("first clue\nsecond clue", text)
        self.assertEqual(text.count("@/lore@1:2"), 2)
