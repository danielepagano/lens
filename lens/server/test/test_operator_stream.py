...
@pytest.fixture
def write_client(tmp_path: Path) -> Generator[TestClient, None, None]:
    from lens.core.project import ProjectSession
    from lens.server.main import create_app
...
class TestOperatorStreamWorkflow:
    def test_write_stream_emits_workflow_plan(self, write_client: TestClient) -> None:
        ...
        with patch(
            ...
            events = _drain_sse_events(
                write_client,
                "/test/operator/write",
                {"prompt": "go", "pins": [], "unpins": []},
            )

        workflow = [e for e in events if e.get("type") == "workflow"]
        assert any(e.get("action") == "plan" for e in workflow)
        assert any(
            e.get("action") == "step_start" and e.get("step_id") == "generate"
            for e in workflow
        )
        done = next(e for e in events if e.get("type") == "done")
        assert done.get("workflow_outcome") == "success"

    def test_workflow_action_without_stream_is_400(self, write_client: TestClient) -> None:
        r = write_client.post(
