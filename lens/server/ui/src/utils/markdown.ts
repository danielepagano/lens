...
export function quotePillHslVars(label: string): { accent: string; border: string } {
...
/** Parse chat annotation YAML lines between `[chat:id` and `]: #` (as_kb_id, with_kb_id). */
export function parseChatDividerParams(paramBlock: string): ParsedChatDividerParams {
  const out: ParsedChatDividerParams = {}
 ...
 * `chat:as-with` after the title (no duplicate inline type pill);
...
export interface MarkdownRendererOptions {
  /** When true, every rendered `<a>` gets `target="_blank"` (e.g. KB viewer). */
...
export function createMarkdownRenderer(options?: MarkdownRendererOptions): MarkdownIt {
  const openLinksInNewTab = options?.openLinksInNewTab ?? false
  ...
  md.renderer.rules.link_open = (tokens, idx, options, env, self) => {
    ...
    const isMountLink =
      href?.includes('/mount/file/') === true || href?.includes('/mount/preview/') === true
    if (openLinksInNewTab || isMountLink) {
  ...
  md.renderer.rules.fence = (tokens, idx, options, env, self) => {
    ...
    if (token && fenceLang(token.info) === 'tool-result') {
      return renderToolCallFence(md, token.content)
    }
