...
Caps how many mount files `lens media search` (and the `media_search` LLM tool) keeps a precomputed search record for, in-memory, per project. Default `10000`. Files beyond the cap still appear in results — they're just matched on demand instead of via the fast path, so the cap only trades off search latency at very large scales, never correctness.
...
> where a GitHub Actions / GitLab CI pipeline runs ``release.sh`` to deploy.
> ``lens release check``/``apply`` emit the CI contract; ``lens deploy init``
> is still needed for first-time Fly app/volume creation.  See
> ``deploy/README.md`` for the full distinction.
...
| `requested_version` | No | `""` | Explicit version to target (e.g. `"2.1.0"`); set by the UI's **Update** button and cleared via `POST /release/clear` or `lens release clear` |
