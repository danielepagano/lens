...
**A template** (`<type>._template`) is the shape of a stored object: fields, ordering, tag policy. Templates and modules overlap by design — the template is the target, the module is how to get there well. One thing to be clear about: the objects a design session writes do **not** pass through the template machinery — `kb_add` takes a body the model composed. The template is *just more prompt*. That is an advantage, not a gap: it means several templates can be read in one session and their artifacts landed in a single object, whenever that is the only place they are ever relevant.
...
The user does not have to have predicted the set. **Scope is the request, not the module list**: if the ask spans a type whose module is not loaded — "a boss you build plus guards from the book" — the operator fetches it with `kb_get design.<key>`, which brings its template and its `rules.<key>` booklet, and builds that object in the same session. What is still declined is the object the *operator* thought of: that gets named and waits for the user. The test is whose idea an object was, not which module happens to be pinned. Without that path the rule contradicted itself, since an encounter cannot be finished around a stat block nobody is allowed to write.
...
Lens de-duplicates the result. Rules for a type arrive automatically for objects actually in scope — an encounter containing `stat.*` blocks brings `rules.stat` with no tagging and nothing to remember, and `--module stat` brings the same booklet to the session authoring the block — and a booklet reached twice through two different links is still rendered once.
...
- **`rules.<type>` companions.** `<type>._template` tells `design` how to *create* an object of a type; `rules.<type>` tells whoever is holding one how to *use* it. It reaches every consumer by naming convention, never by a tag — and it is **engine behaviour, not a play feature**: `crawl()` runs `RulesCompanionTransform` for every crawl-based operator, so a `write` beat or a `chat` turn with an `encounter.*` object in scope gets `rules.encounter` exactly as a play beat does. Separately, `ModuleTransform` adds `rules.<key>` to a `design --module <key>` session alongside `<key>._template`, because the object being authored is authored *against* it — that one keys off the module's own **key**, not its type, so `design --module stat` gets `rules.stat` rather than `rules.design`, and the two paths dedupe against each other. Ship the file and all of it activates. This is where per-type guidance belongs instead of being repeated in every object — and note what that costs: the booklet is now paid on every beat of *any* operator that has one of its objects in scope. The engine-wide list of names like this one is in [configuration.md](configuration.md#reserved-kb-types).
- **Rules on a design module.** Module pins resolve with `+`, so tagging a rules object on `design.<key>` puts it in the crawl when the session opens. Never tag `rules.<key>` itself — that one is the companion above and arrives on its own; tagging it is a second, staleable copy of a rule the machinery already knows. Link only what *every* session of that kind needs — the base ruleset, plus the runtime procedure the authored object will be read against — and **list** the rest inside the module text. `design` has `kb_get`, runs a handful of turns, and knows by then what sort of scene it is building; a user who already knows can skip even that with `--include rules.combat` or an `@rules.chase` in the opening prompt. Linking the whole shelf just moves the always-loaded problem from `play` to `design`.
...
A design session is a narrative sub-node where the work product is KB objects, not story. Each `lens design` call appends an inline block to the sub-node; the LLM uses `kb_get` and `kb_with_tag` to inspect existing objects and `kb_add` / `kb_patch` / `kb_tag` / `kb_remove` to change them. Those calls are **proposals**: visible to every later lookup in the session, persisted into the node as `[kb-op …]: #` comments, and applied to the knowledge store in one transaction by `lens design --end`. See [Pending KB layer](design.md#pending-kb-layer-proposals-not-writes).
