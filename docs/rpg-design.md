...
- **`rules.<type>` companions.** `<type>._template` tells `design` how to *create* an object of a type; `rules.<type>` tells `play` how to *use* one. It reaches both consumers by naming convention, never by a tag: `RulesCompanionTransform` adds it to a play beat whenever any `<type>.*` object is in scope, and `ModuleTransform` adds it to a `design --module <type>` session alongside `<type>._template`, because the object being authored is authored *against* it. The design side is scoped to the module's own type rather than to everything pinned — a prep session's pins are broad and uncurated, and keying off them would make a session's cost depend on whatever the user brought along. Ship the file and both activate. This is where per-type guidance belongs instead of being repeated in every object.
