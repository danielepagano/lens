...
**How does it run**:  
  ...
  1. Builds **context** in two parts: **KB / pins** (as today)—pin ALL fronts that link to the pinned `timeline` on the advance sub-node (e.g. equivalent of `lens kb with-tag timeline.epic --type front --expand --recurse 1`), and `design.front` automatically (the timeline is already pinned by invocation rules). **Narrative** is not a standard `play`-style ancestor crawl; it uses a **narrative slice** from the previous completed `advance` to the cursor (see *Narrative slice vs standard crawl for `advance`* below: how we find that anchor, validate it, and collect the text).
...
### Narrative slice vs standard crawl for `advance`

**Standard crawl** (what `play` relies on) walks the **ancestor chain** toward the cursor and supplies narrative at each level. The design intent is **fractal summarization**: deeper beats are supposed to surface as **rolled-up prose in ancestors** (and pinning trims the KB horizon). Distant material is useful at **coarser resolution**; you do not need every leaf of old branches at every beat.

**`advance` is a different context problem.** It must reason about **what actually happened in the fiction since the calendar last moved**—enough to update fronts, timers, and possible interruptions—so it needs **recent, specific outcomes**, not the same “whole ancestor stack at full weight” assumption. A **calendar window** can also be huge in raw text (many scenes in one in-world day), which fights the context budget even though the slice is “moving” rather than “global.”

**Narrative slice (direction):** treat context as the narrative **between two anchors**—for example from the **completed previous `advance`** for the **same pinned `timeline`** to the **current cursor**—assembled along the **shortest path** in the narrative tree (unique path through the lowest common ancestor). Take the **relevant spans** of each file on that path (when both endpoints live in the same file, that is a **single range** within that file). **Do not** descend into **lateral** subtrees off that path: the contract is that `section`, `play`, and `advance` **summarize upward** into parent narrative, while operators like **`design` are not main-line fiction**; what mattered for the story should already appear in text on the spine. The player is responsible for keeping those summaries accurate (editing is fine). If recap never left a subtree, or play lived only under design, the slice will miss it—**timeline pin drift** breaks the same assumptions.

**Finding the previous `advance` (anchor):** Search must be **scoped to the same `timeline`** that will advance (effective pins at the advance locus); otherwise “previous” is meaningless. Walk **backward in narrative reading order** from the cursor—sibling order is the one Lens already derives from parent markdown (annotation order, then filesystem tie-break). Stop at the **most recent completed** `advance` session (closed on the parent, timeline already updated by that run’s `--end`). If multiple candidates could match editorially, **validate** against the KB: each run records a **starting day** in the `advance` annotation params and an **elapsed** count from the session outcome (explicit `days_elapsed` in the fenced `advance` block when interrupted, otherwise the requested increment). Under the intended rule that **only** `advance --end` moves that timeline, **start + elapsed** for the candidate must equal the timeline’s **current day counter** immediately **before** this new `advance` begins. That pins the anchor to the session that actually landed the calendar on “today.” If there is no prior completed `advance` for that timeline, the slice starts from the **root** (or project-defined narrative start)—first jump has no predecessor.

**Collecting the text:** After the anchor is fixed, define its **end boundary** in the tree (e.g. immediately after the closing `advance` tag and the appended time-passed summary on the parent). Define the **“now” boundary** at the cursor (start of the new `advance` invocation). Walk the **shortest path** between those two positions; for each node on the path, take only the **substring(s)** of that file that lie on the spine (parent text from “after we leave the anchor side” through the child hook that continues the path, and so on). Apply the same stripping/encoding conventions as other operator narrative context where it matters. **Do not** open sibling subtrees off the path—their memorable outcomes are expected to appear as **rollup in ancestor text** on the spine.

This slice replaces a plain standard crawl for `advance` narrative (and aligns with **remember**-style incrementality that starts from the previous `advance`): same anchor and timeline discipline, different assembly than `play`—**path-local, fiction-dense**, still **token-conscious** because lateral detail is delegated to summarized ancestor text rather than to opening every sibling branch.
