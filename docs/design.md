...
The composition is one half of "where am I"; the story is the other, and it is the half a person reads rather than audits. `lens spine` answers it: the chain of nodes from the narrative root down to the cursor — every ancestor's own prose, in a collated tree its summary, then the cursor node's live passage — printed as prose instead of measured. It runs the same `crawl()` and reads the narrative components off it, so comment stripping, in-place mention expansion, and the suppression of an include an ancestor pin already covers are whatever the prompt does, not a second implementation that can drift from it. `--outline` gives the shape and per-node sizes alone; `--text` gives the prose alone, for piping. A node listed but marked empty is the useful negative result: it is on the spine and contributes nothing, which is usually the answer to "why does the model not know that". The two commands share their cursor resolution (`core/commands/cursor_target.py`) for the same reason `explain` and `stats` share operator detection — two reporters that disagree about where the cursor is are worse than one. There is no server route: this is a reading command for the terminal, and the web app already renders the tree.
...
**Command tools ≠ operators.** Operators are invoked by you or the UI with full crawl and persist rules. **Command tools** are small helpers the model may call *inside* one generation loop — read-only KB lookups during `design` (`kb_get`, `kb_list_tags`, `kb_with_tag`), or whitelisted `kb_patch` during remember. They do not spawn other operators. Speed-first operators (`write`, `play`, `chat`) skip the general registry. `compress` is different again: a bespoke `compress_collate` tool returns a line range and Lens runs `collate` in code.

**Per-beat loops vs latching ones.** The reason fast operators skip command tools is the *shape* of the usual loop: an open-ended lookup that can fire on every beat taxes every beat, and the assumption is that everything needed was already funnelled into the prompt. A **latching** tool is different in kind and is allowed there. It offers information the fiction structurally points at but the prompt omits by design, it can be requested **at most once**, and after that it is in scope by ordinary means — so the ceiling is N round trips across a whole session, not N per exchange. **Model-requested modules** are the case that motivated this (see [rpg-design.md](rpg-design.md)): a dataset registers KB objects as modules in its `lens.toml` (`[[dataset.modules]]`, with a target operator; what the module covers and when it is needed comes from the object's own first three lines, never from the manifest); the per-invocation catalog offers only modules **not already in scope** by any route (pin, session module, include, mention, `+` expansion); the model calls `load_module` with one id, gets the content in time to use it in that same reply, and Lens persists an `include` annotation above the block it writes so later beats read it off the node. That position is load-bearing: `write_discard` truncates from the open tag down, so retry and rewind keep the include instead of paying for the call again. Nothing else is persisted: the call leaves **no** `tool-call` fence, because the include already records it and a fence would sit in the assistant turn every later beat reads back, teaching a narrative model a tool name and a fence shape for no gain. (A *failed* call still leaves a `tool-result` audit fence — that one has no include behind it.) The live stream still shows the call, so the pause is explained while it happens. There is no unload tool: `collate` carries the annotations into the child node, and `--end` closes the scope, which is the same answer the structure operators already give.
