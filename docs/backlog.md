...
- **`collate` tool**: uses a range selection (selecting with reference lines and context like `kb_patch` does) over the current node. The selects a range, provides a section slug, and optionally adds a summary refinement prompt (this is helpful for example in `chat` mode when the character itself is controlling what they want to remember), and creates a `collate` operator call; this allows the AI to auto-compress its own context as it goes, but in a purposeful way, as in "well, that topic is over, let's just remember the gist of it"  
   - This is allowed only on a single contiguous range on the current node only (i.e. where the cursor is) because that's where the model has full text visibility, and obviously the selection has to follow the same rules as `collate` operator selection boundaries, which means don't break apart summaries (it can already not see annotations since it targets text lines)
   - Section/node operator summaries appear as a sequence of <!-- section:id -->, h3 headers, and continuous blockquotes (`>`) in the parent node; the tool contract should tell the model not to split summaries (so the range of "section comment until after blockquotes" are off-limits) since we can't "split" a sub-node. This way the collate target ranges stay structurally consistent with what collate allows, without the LLM having to see annotations (which it cannot).
   - The collate tool does NOT immediately perform the collate! That would change the context and cause a mess: it can only be executed after the LLM is done (before then, it just returns whether it was accepted as valid or the request had errors like bad range or already existing slug). 
      - After the LLM is done, we can either automatically execute it or surface the request as a _suggestion_ to the user, who can accept it, redact it, or ignore it; in that case this is a UI artifact and the user mechanically calls the operator.
   - Feasibility and efficacy: we don't want the AI to do this so often that it loses coherence (summaries + memories are not quite as good as the real chat) and ideally it should do it on content that is a already "a bit far way" from the current conversation. To do this, the prompt to use collate would requires clear instructions, but actually doing this may be a TALL ASK for a model, especially if thinking is off. It may be more effective to maybe have a dedicated compression agent (dedicated prompt and tool access, running before main LLM chat call) that looks for sections when the context is large enough and not too close to the cursor, then perform both collation and KB remembering? This could be done every X turns and/or a certain amount of node size increase (the operation can leave a last execution trace/stats in the front matter that we would use easily evaluate that for a trigger with a bit of python... this would be smart because if the user rewinds or manually collates, then our size is smaller, and we don't trigger for no reason?)... this could be better than even the current "all chat calls can remember things" mode?
