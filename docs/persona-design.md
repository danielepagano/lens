...
1. **Immediate memory (`kb_patch`).** Persona-facing KB objects are kept **small** so the model can load a whole object into context whenever it needs the canonical text. Updates should feel like a **coding agent**: apply a patch to that object’s body, not only “emit fenced kb blocks for human review.” Concretely, the tool should accept something in the spirit of a **unified diff** (or an equivalent search/replace hunk): a few lines of unchanged **context**, then `-` / `+` lines for delete/insert. **Append** is the simplest and safest operation: extend the stored markdown with new trailing paragraphs without touching existing text (no matching, no risk of skew). Tag changes can stay a separate, explicit shape if needed. Restrict autonomous patch to an **allowlist** of types or id patterns where mid-session edits are safe. When objects are inlined into prompts, `KnowledgeObject.format` (see `lens/core/knowledge.py`) wraps the body as `KB['type.key']` and **indents every body line by two spaces** for readability. That is fine for *display* but is a poor basis for patches if the model copies that view verbatim; the implementation should either **omit that indentation in the serialized form used for patching** or **strip it on apply** so patches are anchored to the **on-disk** object text. The model never needs line numbers for KB edits—only exact excerpts in the patch, like code.

2. **Contextual collate (no line numbers in the prompt).** The persona can request compression by calling a collate tool with **verbatim excerpts** from the passage it sees, not file line indices: a **start** span (first line or lines of the range) and an **end** span (last line or lines), copied exactly from the current node’s prose. If that pair is ambiguous in the real file, the tool arguments add **before** / **after** lines (extra context above/below) until the host can resolve a **single** contiguous range on the **current node** only; the host then maps that to `(start_line, end_line)` and runs the existing collate operator (which already rejects ranges that would split operator annotations, front matter, etc.). Section summaries and similar material often appear as **continuous blockquotes** (`>`); the tool contract should tell the model **not to split a contiguous quoted run**—so collate targets stay structurally sane even before the operator’s own checks.

   **Turn boundary.** Collate **restructures files and invalidates the crawl** built for that turn. The model must **not** keep generating after a collate in the same response: the runtime applies collate, **rewrites the tree**, **re-crawls**, and the **next** message supplies fresh context. Treat collate as something issued when the model is **done** speaking for that beat (e.g. last tool call in the turn, or a dedicated “finalize” phase), not mid-stream alongside more dialogue.

3. Users can refer to node slices like they do for KB items, so they can say "as we discussed in @/section/sub-section:32-34 you said..." which then copies those lines into the current node as a quote, like a KB reference.

4. **Context size monitoring and compression:** The user should not have to micromanage token count. Both the user and the agent should see rough context size (pinned KB vs narrative history) and be able to collate topics—standard compression—while ensuring important beats have already been **written into KB** via `kb_patch` (or equivalent) so nothing load-bearing lives only in prose that is about to be summarized away.

The above are generally useful core Lens features!
