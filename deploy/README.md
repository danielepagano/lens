...
1. A user clicks **Update** in the Lens UI, which writes `requested_version` +
   `requested_from_commit` into `lens.toml` (uncommitted — decision #3).
...
3. The push triggers the release leader's CI pipeline (`release.sh`), which:
   a. Runs `lens release check --since <SHA>` — parent-hash match (decision #4).
   b. Runs `lens release secrets sync` — reads `[[dependent_project]]` from
      the leader's `lens.toml`, clones each sibling to read its API keys
      and dataset references, and pushes the needed Fly secrets (additive
      only — never deletes).
   c. Runs `flyctl deploy --build-arg LENS_VERSION=<tag>`.
...
| `DEPENDENT_PROJECT_DEPLOY_KEY_<NAME>` | CI secret (optional, per sibling) | SSH deploy key for a `[[dependent_project]]` repo |
...
| File | Purpose |
|------|---------|
| `deploy/ci/release.sh` | Shared shell script: check → secrets sync → apply → deploy |
