...
**First three lines** (`storage_text.kb_headline`, `KnowledgeObject.headline()`): every KB object reserves its opening three lines for its name and what it is for. `kb_with_tag` prints them under every match (not on `--expand`, where bodies are already shown), and `[[dataset.modules]]` uses them as the `load_module` catalog entry instead of a `description` in the manifest. Annotation blocks are stripped before counting; a whole-line HTML comment is unwrapped, which is why type templates put their usage note in `<!-- … -->`. Unenforced by design — the penalty for breaking it is being unfindable.
